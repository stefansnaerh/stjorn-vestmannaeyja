  ...
  if (type === "button") {
    ...
    return (
      <PrismicNextLink
        ...
        aria-label={ariaLabel}
        className={cx(
          `group flex pointer-events-auto rounded-32 border-[1px]  gap-16 py-fluid-8 items-center pr-fluid-12 pl-fluid-24  h-fit w-fit  font-body font-medium transition-all duration-300 ease-in-out ${className} `,
          {
            ["shadow-none"]: noShadow,
            ["shadow-button"]: !noShadow,
          }
      ...
      >
        ...
        {text}
        {arrow ? (
          <div className="p-12 group-hover:bg-pureBlack   w-fit rounded-full transition-all duration-300 ease-in-out">
            <Arrow className="text-pureWhite group-hover:text-cyanideBlue -rotate-45 h-[14px] transition-all duration-300 ease-in-out " />
          </div>
        ) : (
          ""
        )}
