...
export default function Footer() {
  return (
    <footer className=" bg-backgroundGrey px-fluid-24 sm:px-fluid-40 mt-fluid-96 ">
      <div className=" flex bg-white rounded-6  pt-fluid-48 pb-fluid-56  lg:py-fluid-72   w-fill smmd:flex-nowrap smmd:flex-row flex-wrap px-24 xs:px-fluid-40 sm:px-fluid-56 flex-col  gap-y-fluid-48  gap-64 lg2:px-[7%] xl:px-[9%] xxl:px-[12%] ">
        ...
        <div className="flex w-fill lg:max-w-[70%] md:justify-end flex-row flex-wrap gap-fluid-64 gap-y-fluid-48 md:gap-fluid-144 xs:gap-fluid-96  ">
          <nav className="flex overflow-hidden font-body text-bodyGrey flex-wrap   sm:p-0 ">
            <ul
              aria-label={`Hagnýtt`}
              className="flex gap-8 text-navLinks   md:min-h-[160px] w-fit  md:w-[130px] lg:w-fit flex-col font-medium before:content-[attr(aria-label)] before:font-semiBold before:font-headline before:text-md before:text-headlineBlue "
            >
          ...
          <ul
            ...
            className="flex gap-8 text-navLinks font-body text-bodyGrey md:min-h-[160px] w-fit  md:w-[130px] lg:w-fit flex-col  before:content-[attr(aria-label)] before:font-semiBold before:font-headline before:text-md before:text-headlineBlue "
          >
            <li className="hover:text-orange transition-all duration-300 ease-in-out">
              <a
                href={`mailto:stf@stf.is`}
                aria-label={`Opnar tölvupóst til fylla inn í nýjum glugga`}
                target="_blank"
              >
                Netfang: stf@stf.is
            ...
            <li className="hover:text-orange transition-all duration-300 ease-in-out">
              <a href={`tel:5535040`} aria-label={`Hringja í síma fylla inn`}>
                Sími : 553-5040
              </a>
            </li>
      ...
      <div className=" flex w-fill justify-center bg-backgroundGrey py-fluid-24">
        <p className="font-body text-sm text-bodyGrey text-center">
          Ⓒ 2024 Allur réttur áskilinn | STF - Samband stjórnendafélaga{" "}
        </p>
      </div>
    </footer>
  );
